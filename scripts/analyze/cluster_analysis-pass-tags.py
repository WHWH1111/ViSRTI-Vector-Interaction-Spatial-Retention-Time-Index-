...
def load_compound_tags(tag_file_path):
    """
    从CSV文件加载化合物标签
    """
    ...
    for _, row in tag_df.iterrows():
        smiles = row['SMILES']
        tags = row['Tags'].split(',')
        compound_tags[smiles] = tags
    
    return compound_tags

def classify_compounds_by_tags(df, tag_file_path):
    """
    根据预定义的标签文件对化合物进行分类
    返回分类结果和每个化合物的标签列表
    """
    # 加载标签数据
    compound_tags = load_compound_tags(tag_file_path)
    
    classifications = {}
    compound_tag_list = {idx: [] for idx in df.index}
    ...
    for idx, row in df.iterrows():
        ...
        if smiles in compound_tags:
            tags = compound_tags[smiles]
            compound_tag_list[idx] = tags
            for tag in tags:
                classifications[tag].append(idx)
    
    return classifications, compound_tag_list
...
def plot_category_performance(performance_df):
    ...
    # MAE
    axes[0, 0].bar(performance_df['Category'], performance_df['MAE'], color='skyblue')
    axes[0, 0].set_title('Mean Absolute Error by Category')
    axes[0, 0].set_ylabel('MAE')
    axes[0, 0].tick_params(axis='x', rotation=45)
    
    # RMSE
    axes[0, 1].bar(performance_df['Category'], performance_df['RMSE'], color='lightcoral')
    axes[0, 1].set_title('Root Mean Square Error by Category')
    axes[0, 1].set_ylabel('RMSE')
    axes[0, 1].tick_params(axis='x', rotation=45)
    
    # R_squared
    axes[1, 0].bar(performance_df['Category'], performance_df['R_squared'], color='lightgreen')
    axes[1, 0].set_title('R-squared by Category')
    axes[1, 0].set_ylabel('R-squared')
    axes[1, 0].tick_params(axis='x', rotation=45)
    
    # 化合物数量
    axes[1, 1].bar(performance_df['Category'], performance_df['Count'], color='gold')
...
def plot_comprehensive_analysis(performance_df, cross_category_df, category_counts):
    ...
    bars = ax3.bar(range(len(categories)), mae_values, color=bar_colors, edgecolor='black', linewidth=0.5)
    ax3.set_xlabel('Categories')
    ax3.set_ylabel('MAE')
    ax3.set_title('Prediction Performance by Category (MAE)')
    ...
    bars = ax4.bar(range(len(categories)), r2_values, color=bar_colors, edgecolor='black', linewidth=0.5)
    ax4.set_xlabel('Categories')
    ax4.set_ylabel('R²')
    ax4.set_title('Prediction Performance by Category (R²)')
...
def main():
    ...
    # 化合物分类 - 使用预定义标签
    print("正在进行化合物分类...")
    tag_file_path = 'data/MMF-3/tags/neg_compound_tags.csv'
    classifications, compound_tags = classify_compounds_by_tags(df, tag_file_path)
    ...
    if not performance_df.empty:
        ...
        for _, row in sorted_performance.iterrows():
            print(f"   {row['Category']}: MAE={row['MAE']:.2f}, RMSE={row['RMSE']:.2f}, R²={row['R_squared']:.2f}")
    else:
        print("   没有足够的数据计算性能指标")
