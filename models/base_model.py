...
class MolecularGraphNeuralNetwork(nn.Module):
    def __init__(self, N, dim, layer_hidden, layer_output, dropout=0.0):
    ...
    def mlp(self, vectors):
        """ regressor based on multilayer perceptron with dropout."""
        for layer_index in range(len(self.W_output)):
            vectors = torch.relu(self.W_output[layer_index](vectors))
            # 添加dropout层
